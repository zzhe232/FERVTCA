...
def parse_args():
    ...
    parser.add_argument('--FER_path', type=str, default='datasets/SFEW', help='SFEW dataset path.')
...
def run_training():
    ...
    val_dataset = datasets.ImageFolder(f'{args.FER_path}/val', transform=data_transforms_val)
    ...
    criterion_cls = mycs(temperature=1.2,label_smoothing=0.1)
    ...
    for epoch in tqdm(range(1, args.epochs + 1)):
        ...
        with torch.no_grad():
            ...
            if acc > 0.50 and acc == best_acc:
                torch.save({'iter': epoch,
                            'model_state_dict': model.state_dict(),
                            'optimizer_state_dict': optimizer.state_dict(), },
                           os.path.join('checkpoints', "SFEW"+".pth"))
                ...
                plot_confusion_matrix(matrix, classes=class_names, normalize=True,
                                      title='SFEW Confusion Matrix (acc: %0.2f%%)' % (acc * 100))

                plt.savefig(os.path.join('checkpoints', "SFEW_epoch" + str(epoch) + "_acc" + str(acc) + "_bacc" + str(
                    balanced_acc) + ".png"))
                plt.close()
